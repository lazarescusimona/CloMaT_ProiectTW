<?php 
	...
	oci_execute($stid);
	$myfile = fopen($_SERVER['DOCUMENT_ROOT']."/CloMaT_ProiectTW/filtre.csv", "w") or die("Unable to open file!");
   ...
   if(isset($_POST["ExportUtilizatori"])){
    ...
    oci_execute($stid);
    $myfile = fopen($_SERVER['DOCUMENT_ROOT']."/CloMaT_ProiectTW/users.csv", "w") or die("Unable to open file!");
    ...
    if(isset($_POST["ExportArticole"])){
        ...
        oci_execute($stid);
        $myfile = fopen($_SERVER['DOCUMENT_ROOT']."/CloMaT_ProiectTW/articole.csv", "w") or die("Unable to open file!");
        ...
        if(isset($_POST["ExportPreferinte"])){
            ...
            oci_execute($stid);
            $myfile = fopen($_SERVER['DOCUMENT_ROOT']."/CloMaT_ProiectTW/articole_preferate.csv", "w") or die("Unable to open file!");
