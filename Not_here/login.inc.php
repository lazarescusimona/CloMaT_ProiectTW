...
<?php       
        ...
        if(isset($_POST['submit'])){
            ...
            {
              $pass = md5($pass);
              $query = oci_parse($conn, "SELECT * FROM utilizatori WHERE username = '$user' AND parola = '$pass' and confirmed_mail =1");
              oci_execute($query);
              if( $rows = oci_fetch_array($query) )
              {
                ...
                if($rows[8] == 'user'){
                  ...
                  if($rows[8] == 'admin')
                    header("location: http://localhost/CloMaT_ProiectTW/admin.php");
                }
              }
              else{
              //Redirectionam pe login.php?signup
              header("Location: http://localhost/CloMaT_ProiectTW/login.php?signup=notexists"); // eroarea signup=notexists va fi preluata in login.php
              exit();
            }
            }
        }
